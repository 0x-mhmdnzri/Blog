...
public class HomeController : Controller
{
        ...
        string? partial = null)
    {
        ...
        ViewBag.HomeSeries = await _db.PostSeries.AsNoTracking()
            .Where(s => s.Posts.Any(sp => sp.Post.IsPublished && !sp.Post.IsDeleted
                && (sp.Post.ExpiresAtUtc == null || sp.Post.ExpiresAtUtc > nowUtc)
                && sp.Post.LanguageCode == lang))
            .OrderByDescending(s => s.Posts.Count(sp => sp.Post.IsPublished && !sp.Post.IsDeleted))
            .Take(8)
            .Select(s => new BlogFeedSeriesItem
            {
                Name = s.Name,
                Slug = s.Slug,
                Count = s.Posts.Count(sp => sp.Post.IsPublished && !sp.Post.IsDeleted && sp.Post.LanguageCode == lang)
            })
            .ToListAsync();
