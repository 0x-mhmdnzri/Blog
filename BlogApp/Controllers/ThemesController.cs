...
public class ThemesController : Controller
{
    ...
    /// <summary>SuperAdmin review-only theme id (browser-scoped).</summary>
    public const string PreviewCookie = "Blog.ThemePreview";
    ...
    {
        // 0) SuperAdmin review preview (any approval status) — AdminThemes/Preview sets cookie
        if (Request.Cookies.TryGetValue(PreviewCookie, out var prevRaw)
            && int.TryParse(prevRaw, out var prevId) && prevId > 0)
        {
            var preview = await _themes.GetByIdAsync(prevId);
            if (preview is not null)
            {
                var cssP = $":root{{{ThemeContrastService.ToCssVariables(preview)}}}html{{color-scheme:{preview.Mode}}}";
                Response.Headers.CacheControl = "private, no-store";
                Response.Headers["X-Theme-Preview"] = prevId.ToString();
                return Content(cssP, "text/css; charset=utf-8");
            }
        }
    ...
    public async Task<IActionResult> ImportFile(IFormFile? file, string? action)
    {
        ...
        TempData["Msg"] = entity.Status == ThemeApprovalStatus.Pending
            ? $"فایل «{entity.Name}» بارگذاری و برای تأیید ارسال شد."
            : $"فایل «{entity.Name}» به‌عنوان پیش‌نویس ذخیره شد.";
        return RedirectToAction(nameof(Index));
    }
