...
using AVICRM.Data;
using AVICRM.Developer;
using AVICRM.Developer.Middleware;
using AVICRM.Logging;
using AVICRM.Middleware;
using AVICRM.Models;
...
try
{
    ...
    builder.Services.AddAppBackup(builder.Configuration);
    ...
    Log.Information("AVICRM starting Environment={Environment} ContentRoot={ContentRoot}",
    ...
    Log.Information("AVICRM monolith listening ForceHttps={ForceHttps} MassTransit EDD enabled; ContentSchedule + IndexNow + Comments spam registered",
        forceHttps);
    app.Run();
}
catch (Exception ex)
