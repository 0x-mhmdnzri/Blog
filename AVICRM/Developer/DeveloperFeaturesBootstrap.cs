...
using AVICRM.Developer.Middleware;
using AVICRM.Developer.Observability;
using AVICRM.Developer.Plugins;
using AVICRM.Developer.Widgets;
using MassTransit;
...
public static class DeveloperFeaturesBootstrap
{
    public static IServiceCollection AddDeveloperFeatures(this IServiceCollection services, IConfiguration config)
    {
        ...
        services.AddMassTransit(x =>
        {
            ...
            x.AddConsumer<AVICRM.Services.Messaging.NotifyOnPostPublishedConsumer>();
            x.AddConsumer<AVICRM.Services.Messaging.NotifyOnCommentApprovedConsumer>();
            x.AddConsumer<AVICRM.Services.Messaging.NotifyOnAuthorFollowedConsumer>();
            x.AddConsumer<AVICRM.Services.Messaging.WebhookDispatchConsumer>();
        ...
        services.AddHealthChecks()
            .AddCheck("self", () => HealthCheckResult.Healthy("AVICRM"), tags: new[] { "live" })
            .AddCheck<SqliteHealthCheck>("sqlite", tags: new[] { "ready" });

        // ── OpenTelemetry ───────────────────────────────────────────────────
        var serviceName = config["OpenTelemetry:ServiceName"] ?? "AVICRM";
