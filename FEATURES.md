...
- [x] **P3.1** Content update cadence for pages we want recrawled often — *shipped 2026-08-07*
  - Dynamic `changefreq` / `priority` in sitemaps by age of `lastmod` (hourly/daily/weekly/monthly)
  - Google News sitemap (`sitemap-news.xml`) for posts published in last 48h
  - IndexNow already pings on publish; `lastmod` reflects real `UpdatedAtUtc`
  - Homepage / language roots stay `daily` priority 0.9–1.0
- [x] **P3.2** Structured data + mobile rendering parity (Googlebot Smartphone) — *shipped 2026-08-07*
  - JSON-LD Article/BlogPosting with `dateModified`, image 1200×630, view `InteractionCounter`, speakable
  - Shared `_SocialMeta` OG + Twitter Cards (LinkedIn, Telegram, WhatsApp, X, Facebook, Discord)
  - GitHub-style OG cards (`/og/post|author`) with views, category, tags, read time
  - Viewport + `theme-color` + `apple-mobile-web-app-capable` + `format-detection`
  - `max-image-preview:large` robots directive; responsive layout (no desktop-only bot trap)
